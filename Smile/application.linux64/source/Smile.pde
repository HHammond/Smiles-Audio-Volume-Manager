...
void drawLight(float happiness) {
  ...
  ellipse( offsetX+int(w*1.0/2), offsetY+int(h*1.0/3)-rad/2, rad-10, rad-10);

  //yellow
  fill( happiness<YELLOW && happiness>GREEN ? color(255, 255, 0):color(61, 61, 41));
  ellipse( offsetX+int(w*1.0/2), offsetY+int(h*2.0/3)-rad/2, rad-10, rad-10);

  //green
  fill( happiness<=GREEN ? color(18, 170, 64):color(61, 61, 41));
  ellipse( offsetX+int(w*1.0/2), offsetY+int(h*3.0/3)-rad/2, rad-10, rad-10);
}

void drawFace(float happiness) {
  ...
  //draw eyes
  fill( hueLevel, 240, 0);
  float eyeRadius = faceRadius*0.08;
  float eyeOffsetX = faceRadius*0.15;
  float eyeOffsetY = faceRadius*0.1;
  ellipse( faceX-eyeOffsetX, faceY-eyeOffsetY, eyeRadius, eyeRadius);
  ellipse( faceX+eyeOffsetX, faceY-eyeOffsetY, eyeRadius, eyeRadius);

  //draw mouth
  float mouthHeight = faceRadius*.12;
  float mouthWidth = faceRadius*.4;
  float mouthOffset = faceRadius*0.25;
  
  noFill();
  
  bezier( faceX-mouthWidth/2, faceY+mouthOffset+mouthHeight*(0.5-happiness), 
  faceX-mouthWidth*0.25, faceY+mouthOffset+mouthHeight*(happiness-0.5)*1.1, 
  faceX+mouthWidth*0.25, faceY+mouthOffset+mouthHeight*(happiness-0.5)*1.1, 
  faceX+mouthWidth/2, faceY+mouthOffset+mouthHeight*(0.5-happiness)
    );
}
